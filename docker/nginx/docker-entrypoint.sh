...
if [ -n "$PHP_SERVICE" ]; then
    ...
    # IMPORTANTE: Remover TODOS os arquivos .conf exceto default-simple.conf e default-ssl-simple.conf para evitar conflitos
    # O nginx carrega TODOS os arquivos .conf do diretório http.d/
    # Se my-site.conf ou default-ssl.conf estiverem presentes, causarão conflitos
    ...
    if [ -f /etc/nginx/http.d/default-ssl-simple.conf ]; then
        ...
        # Remover o arquivo template SSL
    ...
    # Remover o arquivo template HTTP para evitar que o nginx tente carregá-lo
