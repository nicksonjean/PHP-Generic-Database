 ...
 *
 * Nota: PDO pode variar conforme driver (SQL Server, MySQL, SQLite, etc.).
...
$qbUnion = (new PDOQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new PDOQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new PDOQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new PDOQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new PDOQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new PDOQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new PDOQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new PDOQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new PDOQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new PDOQueryBuilder($context))
...
$qbNotExists = (new PDOQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new PDOQueryBuilder($context))
...
$qbComplex = (new PDOQueryBuilder($context))
...
$qbUnionAllTest13 = (new PDOQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new PDOQueryBuilder($context))->select('id', 'nome')->from('cidade')
