...
$sqlUnion = 'SELECT nome FROM estado UNION SELECT nome FROM cidade ORDER BY nome FETCH FIRST 15 ROWS ONLY';
...
$sqlUnionAll = 'SELECT nome FROM estado UNION ALL SELECT nome FROM cidade ORDER BY nome FETCH FIRST 10 ROWS ONLY';
...
$sqlExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome FETCH FIRST 5 ROWS ONLY';
...
$sqlNotExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE NOT EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome FETCH FIRST 5 ROWS ONLY';
...
$sqlPrepareUnion = 'SELECT nome FROM (SELECT nome FROM estado UNION SELECT nome FROM cidade) u ORDER BY nome FETCH FIRST :lim ROWS ONLY';
...
$sqlPrepareExists = 'SELECT e.id, e.nome FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome FETCH FIRST :lim ROWS ONLY';
...
$qbUnion = (new OCIQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new OCIQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new OCIQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new OCIQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new OCIQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new OCIQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new OCIQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new OCIQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new OCIQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new OCIQueryBuilder($context))
...
$qbNotExists = (new OCIQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new OCIQueryBuilder($context))
...
$qbComplex = (new OCIQueryBuilder($context))
...
$sqlUnionAllUnquoted = 'SELECT id, nome FROM estado UNION ALL SELECT id, nome FROM cidade ORDER BY nome FETCH FIRST 8 ROWS ONLY';
...
$qbUnionAllTest13 = (new OCIQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new OCIQueryBuilder($context))->select('id', 'nome')->from('cidade')
