...
$test0 = (new JSONQueryBuilder($context))->select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from(['estado e'])
    ->where(['e.id >= 25']);
...
$testA = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from(['estado e'])
    ->where(['e.id <= 25']);
...
$testB = (new JSONQueryBuilder($context))->select('e.id AS Codigo, e.nome AS Estado, e.sigla AS Sigla')
    ->from('estado e')
    ->where('e.id >= 1')
    ->andWhere('e.id <= 5');
...
$testC = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from('estado e')
    ->where('e.id = 6')
    ->orWhere('e.id = 11');
...
$testD = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from('estado e')
...
$testE = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from('estado e')
...
$testF = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from('estado e')
...
$testF1 = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from('estado e')
...
$testG = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'e.sigla AS Sigla'])
    ->from('estado e')
...
$testH1 = JSONQueryBuilder::with($context)::select(['e.id AS Codigo', 'e.nome AS Estado', 'COUNT(c.id) as num_cidades'])
    ->from(['estado e'])
    ->join(['cidade c'])
    ->on(['e.id = c.estado_id'])
    ->group(['e.id, e.nome'])
...
$testH2 = JSONQueryBuilder::with($context)::select('e.id AS Codigo, e.nome AS Estado, COUNT(c.id) as num_cidades')
    ->from('estado e')
    ->join('cidade c')
    ->on('e.id = c.estado_id')
    ->group('e.id, e.nome')
...
$testI = JSONQueryBuilder::with($context)::select(['e.*', 'c.*'])
...
$testI1 = JSONQueryBuilder::with($context)::select('e.*, c.*')
    ->from('estado e, cidade c')
    ->where('e.id = 5')
    ->andWhere('c.id = 5');
...
$testJ = JSONQueryBuilder::with($context)::select(['e.*', 'c.*'])
    ->from(['estado e', 'cidade c'])
...
$testJ1 = JSONQueryBuilder::with($context)::select(['e.*', 'c.*'])
    ->from(['estado e', 'cidade c'])
...
$testJ2 = JSONQueryBuilder::with($context)::select('e.*, c.*')
    ->from('estado e, cidade c')
    ->where([['e.id = 10'], ['AND' => 'e.id = 11'], ['AND' => 'e.id = 12']])
    ->andWhere([['e.id = 13'], ['OR' => 'c.id = 14']])
    ->order(['c.id DESC'])
