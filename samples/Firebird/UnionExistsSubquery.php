...
$sqlUnion = 'SELECT nome FROM estado UNION SELECT nome FROM cidade ORDER BY 1 ROWS 15';
...
$sqlUnionAll = 'SELECT nome FROM estado UNION ALL SELECT nome FROM cidade ORDER BY 1 ROWS 10';
...
$sqlExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY 1 ROWS 5';
...
$sqlNotExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE NOT EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY 1 ROWS 5';
...
$sqlPrepareUnion = 'SELECT nome FROM (SELECT nome FROM estado UNION SELECT nome FROM cidade) u ORDER BY 1 ROWS :lim';
...
$sqlPrepareExists = 'SELECT e.id, e.nome FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY 1 ROWS :lim';
...
$qbUnion = (new FirebirdQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new FirebirdQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('1 ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new FirebirdQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new FirebirdQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('1 ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new FirebirdQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new FirebirdQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('1 ASC')
    ->limit('0, 8');
...
$qbExists = (new FirebirdQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new FirebirdQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new FirebirdQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new FirebirdQueryBuilder($context))
...
$qbNotExists = (new FirebirdQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new FirebirdQueryBuilder($context))
...
$qbComplex = (new FirebirdQueryBuilder($context))
...
$qbUnionAllTest13 = (new FirebirdQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new FirebirdQueryBuilder($context))->select('id', 'nome')->from('cidade')
