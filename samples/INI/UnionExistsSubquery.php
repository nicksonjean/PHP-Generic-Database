...
/**
 * Casos de uso: EXISTS, UNION, UNION ALL e Subqueries (Flat File - INI).
 ...
 * Fontes: estado, cidade (estado.ini, cidade.ini)
 */
...
$qbUnion = (new INIQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new INIQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new INIQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new INIQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new INIQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new INIQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new INIQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new INIQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new INIQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new INIQueryBuilder($context))
...
$qbNotExists = (new INIQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new INIQueryBuilder($context))
...
$qbComplex = (new INIQueryBuilder($context))
...
$qbUnionAllTest13 = (new INIQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new INIQueryBuilder($context))->select('id', 'nome')->from('cidade')
