 ...
 *
 * Nota: ODBC pode variar conforme driver (SQL Server, MySQL, SQLite, etc.).
 * Ajuste a sintaxe SQL se necessário para o seu driver.
 */
...
$sqlPrepareUnion = 'SELECT nome FROM (SELECT nome FROM estado UNION SELECT nome FROM cidade) u ORDER BY nome LIMIT :lim';
...
$qbUnion = (new ODBCQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new ODBCQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new ODBCQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new ODBCQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new ODBCQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new ODBCQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new ODBCQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new ODBCQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new ODBCQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new ODBCQueryBuilder($context))
...
$qbNotExists = (new ODBCQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new ODBCQueryBuilder($context))
...
$qbComplex = (new ODBCQueryBuilder($context))
...
$qbUnionAllTest13 = (new ODBCQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new ODBCQueryBuilder($context))->select('id', 'nome')->from('cidade')
