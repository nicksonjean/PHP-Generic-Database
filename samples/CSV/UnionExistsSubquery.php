...
/**
 * Casos de uso: EXISTS, UNION, UNION ALL e Subqueries (Flat File - CSV).
 ...
 * Fontes: estado, cidade (estado.csv, cidade.csv)
 */
...
$qbUnion = (new CSVQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new CSVQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new CSVQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new CSVQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new CSVQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new CSVQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new CSVQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new CSVQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new CSVQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new CSVQueryBuilder($context))
...
$qbNotExists = (new CSVQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new CSVQueryBuilder($context))
...
$qbComplex = (new CSVQueryBuilder($context))
...
$qbUnionAllTest13 = (new CSVQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new CSVQueryBuilder($context))->select('id', 'nome')->from('cidade')
