 ...
 * Usa QueryBuilder genérico (facade) - troque a conexão para testar com qualquer engine.
 *
...
$sqlUnion = 'SELECT nome FROM estado UNION SELECT nome FROM cidade ORDER BY nome LIMIT 10';
...
$sqlExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) LIMIT 5';
...
$sqlNotExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE NOT EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) LIMIT 5';
...
$sqlPrepareExists = 'SELECT e.id, e.nome FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) LIMIT :lim';
...
$qbUnion = (new QueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new QueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new QueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new QueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new QueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new QueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new QueryBuilder($context))
...
$qbExists2 = (new QueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new QueryBuilder($context))
...
$qbNotExists = (new QueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new QueryBuilder($context))
...
$qbComplex = (new QueryBuilder($context))
