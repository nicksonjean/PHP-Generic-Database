...
/**
 * Casos de uso: EXISTS, UNION, UNION ALL e Subqueries.
 ...
 * Tabelas: estado, cidade (cidade.estado_id -> estado.id)
 */
...
$context = Chainable::nativeSQLSrv(env: $_ENV, strategy: false)->connect();
...
$sqlUnion = 'SELECT nome FROM estado UNION SELECT nome FROM cidade ORDER BY nome OFFSET 0 ROWS FETCH NEXT 15 ROWS ONLY';
...
$sqlUnionAll = 'SELECT nome FROM estado UNION ALL SELECT nome FROM cidade ORDER BY nome OFFSET 0 ROWS FETCH NEXT 10 ROWS ONLY';
...
$sqlExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY';
...
$sqlNotExists = 'SELECT e.id, e.nome, e.sigla FROM estado e WHERE NOT EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY';
...
$sqlPrepareUnion = 'SELECT nome FROM (SELECT nome FROM estado UNION SELECT nome FROM cidade) AS u ORDER BY nome OFFSET 0 ROWS FETCH NEXT :lim ROWS ONLY';
...
$sqlPrepareExists = 'SELECT e.id, e.nome FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome OFFSET 0 ROWS FETCH NEXT :lim ROWS ONLY';
...
$qbUnion = (new SQLSrvQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new SQLSrvQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new SQLSrvQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new SQLSrvQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new SQLSrvQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new SQLSrvQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new SQLSrvQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new SQLSrvQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new SQLSrvQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new SQLSrvQueryBuilder($context))
...
$qbNotExists = (new SQLSrvQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new SQLSrvQueryBuilder($context))
...
$qbComplex = (new SQLSrvQueryBuilder($context))
...
$sqlUnionAllUnquoted = 'SELECT id, nome FROM estado UNION ALL SELECT id, nome FROM cidade ORDER BY nome OFFSET 0 ROWS FETCH NEXT 8 ROWS ONLY';
...
$qbUnionAllTest13 = (new SQLSrvQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new SQLSrvQueryBuilder($context))->select('id', 'nome')->from('cidade')
