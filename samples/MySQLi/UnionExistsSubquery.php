...
$sqlPrepareUnion = 'SELECT nome FROM (SELECT nome FROM estado UNION SELECT nome FROM cidade) u ORDER BY nome LIMIT ?';
$stmtPrepare = $context->prepare($sqlPrepareUnion, [8]);
...
$sqlPrepareExists = 'SELECT e.id, e.nome FROM estado e WHERE EXISTS (SELECT 1 FROM cidade c WHERE c.estado_id = e.id) ORDER BY e.nome LIMIT ?';
$stmtPrepareExists = $context->prepare($sqlPrepareExists, [3]);
...
$qbUnion = (new MySQLiQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union(
        (new MySQLiQueryBuilder($context))->select('nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 10');
...
$qbUnion2 = (new MySQLiQueryBuilder($context))
    ->select('nome')
    ->from('estado')
    ->union((new MySQLiQueryBuilder($context))->select('nome')->from('cidade'))
    ->order('nome ASC')
    ->limit('0, 5');
...
$qbUnionAll = (new MySQLiQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new MySQLiQueryBuilder($context))->select('id', 'nome')->from('cidade')
    )
    ->order('nome ASC')
    ->limit('0, 8');
...
$qbExists = (new MySQLiQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new MySQLiQueryBuilder($context))
            ->select('1')
            ->from('cidade c')
...
$qbExists2 = (new MySQLiQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereExists(
        (new MySQLiQueryBuilder($context))
...
$qbNotExists = (new MySQLiQueryBuilder($context))
    ->select('e.id', 'e.nome', 'e.sigla')
    ->from('estado e')
    ->whereNotExists(
        (new MySQLiQueryBuilder($context))
...
$qbComplex = (new MySQLiQueryBuilder($context))
...
$sqlUnionAllUnquoted = 'SELECT id, nome FROM estado UNION ALL SELECT id, nome FROM cidade ORDER BY nome LIMIT 8';
...
$qbUnionAllTest13 = (new MySQLiQueryBuilder($context))
    ->select('id', 'nome')
    ->from('estado')
    ->unionAll(
        (new MySQLiQueryBuilder($context))->select('id', 'nome')->from('cidade')
