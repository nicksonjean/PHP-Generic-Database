...
use GenericDatabase\Engine\INI\QueryBuilder\Builder;
use GenericDatabase\Engine\INI\QueryBuilder\Clause;

/**
 * The `INIQueryBuilder` class implements the `IQueryBuilder` interface to provide a flexible
 * query building mechanism for INI flat file databases.
 *
 * @package GenericDatabase\Engine
 */
class INIQueryBuilder implements IQueryBuilder
{
    use Query;
    use Context;
    use Singleton;

    private static INIQueryBuilder $self;
    ...
    private function runOnce(): void
    {
        ...
        if (self::$lastQuery !== $currentQueryForDisplay || self::$cursorExhausted) {
            ...
            $rows = $conn->fetchAll(INI::FETCH_ASSOC);
    ...
    private function formatRow(mixed $row, int $fetchStyle, mixed $fetchArgument = null, mixed $optArgs = null): mixed
    {
        ...
        return match ($fetchStyle) {
            INI::FETCH_NUM => array_values($row),
            INI::FETCH_BOTH => $this->formatBothMode($row),
            INI::FETCH_OBJ => (object) $row,
            INI::FETCH_COLUMN => $fetchArgument !== null
                ? ($row[$fetchArgument] ?? array_values($row)[0] ?? null)
                : (array_values($row)[0] ?? null),
            INI::FETCH_CLASS => $this->fetchClass($row, $fetchArgument, $optArgs),
            INI::FETCH_INTO => $this->fetchInto($row, $fetchArgument),
            default => $row,
        };
    }
