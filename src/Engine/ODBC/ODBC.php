...
class ODBC
{
     ...
     */
    final public const ATTR_ALIAS_TYPE = 1101;

    /**
     * Connection attribute to set the connection persistence.
     */
     ...
     */
    private const LINUX_CMDLET = '\bin\odbcinst.bat';
     ...
     * @return mixed the typed input variable
     */
    public static function setType(mixed $input): mixed
    {
        return match (true) {
     ...
     * @return array The array of rows fetched from the result set.
     */
    ...
    {
        $command = sprintf('%s', dirname(__DIR__, 3) . self::WIN_CMDLET);
    ...
    {
        $command = sprintf('%s', dirname(__DIR__, 3) . self::LINUX_CMDLET);
    ...
    {
        $command = sprintf('%s %s', dirname(__DIR__, 3) . self::WIN_CMDLET, "-j");
        return json_decode(shell_exec($command), true);
    }
