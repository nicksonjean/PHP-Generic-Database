...
class Arguments
{
     ...
     */
    private static function callArgumentsByFormat(string $format, mixed $arguments): ODBCConnection
    {
        $data = match ($format) {
            ...
            foreach ($data as $key => $value) {
                if (mb_strtolower($key) === 'options') {
                    call_user_func_array(
                        [ODBCConnection::getInstance(), 'set' . ucfirst($key)],
     ...
     * @return ODBCConnection
     */
    private static function callWithByStaticArray(array $arguments): ODBCConnection
    {
        foreach ($arguments as $key => $value) {
            call_user_func_array([ODBCConnection::getInstance(), 'set' . ucfirst($key)], [$value]);
        }
        return ODBCConnection::getInstance();
    }
     ...
     * @noinspection PhpUnused
     */
    private static function callWithByStaticArgs(array $arguments): ODBCConnection
    {
     ...
     * @throws ReflectionException
     */
    public static function call(string $name, array $arguments): ODBCConnection
    {
        ...
        return match ($name) {
            ...
            default => call_user_func_array([ODBCConnection::getInstance(), $name], $arguments)
        };
    }
}
