...
class Arguments
{
     ...
     * @throws ReflectionException
     */
    private static function callArgumentsByFormat(string $format, mixed $arguments): ODBCEngine
    {
        $data = match ($format) {
            ...
            foreach ($data as $key => $value) {
                if (mb_strtolower($key) === 'options') {
                    call_user_func_array(
                        [ODBCEngine::getInstance(), 'set' . ucfirst($key)],
                        [self::setConstant(($format === 'json' || $format === 'yaml') ? $value : [$value])]
                    );
                } else {
                    call_user_func_array([ODBCEngine::getInstance(), 'set' . ucfirst($key)], [self::setType($value)]);
                }
            }
        }
        return ODBCEngine::getInstance();
    }
     ...
     * @return ODBCEngine
     */
    private static function callWithByStaticArray(array $arguments): ODBCEngine
    {
        foreach ($arguments as $key => $value) {
            call_user_func_array([ODBCEngine::getInstance(), 'set' . ucfirst($key)], [$value]);
        }
        return ODBCEngine::getInstance();
    }
     ...
     * @return ODBCEngine
     */
    private static function callWithByStaticArgs(array $arguments): ODBCEngine
    {
     ...
     * @throws ReflectionException
     */
    public static function call(string $name, array $arguments): ODBCEngine
    {
        ...
        return match ($name) {
            ...
            default => call_user_func_array([ODBCEngine::getInstance(), $name], $arguments)
        };
    }
}
