...
class Arguments
{
     ...
     */
    private static function callArgumentsByFormat(string $format, mixed $arguments): SQLiteConnection
    {
        $data = match ($format) {
            ...
            foreach ($data as $key => $value) {
                if (mb_strtolower($key) === 'options') {
                    call_user_func_array(
                        [SQLiteConnection::getInstance(), 'set' . ucfirst($key)],
     ...
     * @return SQLiteConnection
     */
    private static function callWithByStaticArray(array $arguments): SQLiteConnection
    {
        foreach ($arguments as $key => $value) {
            call_user_func_array([SQLiteConnection::getInstance(), 'set' . ucfirst($key)], [$value]);
        }
        return SQLiteConnection::getInstance();
    }
     ...
     * @noinspection PhpUnused
     */
    private static function callWithByStaticArgs(array $arguments): SQLiteConnection
    {
     ...
     * @throws ReflectionException
     */
    public static function call(string $name, array $arguments): SQLiteConnection
    {
        ...
        return match ($name) {
            ...
            default => call_user_func_array([SQLiteConnection::getInstance(), $name], $arguments)
        };
    }
}
