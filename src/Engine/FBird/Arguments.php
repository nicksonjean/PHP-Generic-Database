...
class Arguments
{
  ...
  private static function setConstant($value): array
  {
    ...
    foreach (array_combine(array_keys(...$value), array_values(...$value)) as $key => $value) {
      $index = str_replace('FBird::', '', $key);
      $key_name = $index !== 'ATTR_PERSISTENT' && $index !== 'ATTR_CONNECT_TIMEOUT' ? str_replace("ATTR", "FBird", $index) : $index;
      FBirdEngine::getInstance()->setAttribute($key, $value);
      if ($index !== 'ATTR_PERSISTENT' && $index !== 'ATTR_CONNECT_TIMEOUT') {
        FBirdEngine::getInstance()->setOptions(constant($key_name), $value);
      }
  ...
  private static function callArgumentsByJSON($arguments): void
  {
    foreach (JSON::parseJSON(...$arguments) as $key => $value) {
      if (strtolower($key) === 'options') {
        call_user_func_array([FBirdEngine::getInstance(), 'set' . ucfirst($key)], [self::setConstant($value)]);
      } else {
