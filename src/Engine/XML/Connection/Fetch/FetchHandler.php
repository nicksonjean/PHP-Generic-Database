...
namespace GenericDatabase\Engine\XML\Connection\Fetch;
...
/**
 * Handles fetch operations for XML connections.
 * Extends AbstractFlatFileFetch to leverage common flat-file fetch functionality.
 *
 * @package GenericDatabase\Engine\XML\Connection\Fetch
 */
class FetchHandler extends AbstractFlatFileFetch implements IFlatFileFetch
{
    ...
    {
        $fetch = $fetchStyle ?? XML::FETCH_ASSOC;

        return match ($fetch) {
            XML::FETCH_OBJ => $this->internalFetchAssoc() !== false ? (object) $this->getResultSet()[$this->cursor - 1] : false,
            XML::FETCH_INTO => $this->fetchIntoObject($fetchArgument),
            XML::FETCH_CLASS => $this->internalFetchClass($optArgs, $fetchArgument ?? '\stdClass'),
            XML::FETCH_COLUMN => $this->internalFetchColumn($fetchArgument ?? 0),
            XML::FETCH_ASSOC => $this->internalFetchAssoc(),
            XML::FETCH_NUM => $this->internalFetchNum(),
    ...
    public function fetchAll(?int $fetchStyle = null, mixed $fetchArgument = null, mixed $optArgs = null): array|bool
    {
        ...
        return match ($fetch) {
            XML::FETCH_OBJ => array_map(fn($row) => (object) $row, $this->internalFetchAllAssoc()),
            XML::FETCH_INTO => $this->fetchAllIntoObjects($fetchArgument),
            XML::FETCH_CLASS => $this->internalFetchAllClass($optArgs, $fetchArgument ?? '\stdClass'),
            XML::FETCH_COLUMN => $this->internalFetchAllColumn($fetchArgument ?? 0),
            XML::FETCH_ASSOC => $this->internalFetchAllAssoc(),
            XML::FETCH_NUM => $this->internalFetchAllNum(),
