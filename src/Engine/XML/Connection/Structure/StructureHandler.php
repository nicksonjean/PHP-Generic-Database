...
namespace GenericDatabase\Engine\XML\Connection\Structure;

use AllowDynamicProperties;
use GenericDatabase\Interfaces\IConnection;
use GenericDatabase\Shared\Run;
use GenericDatabase\Helpers\Exceptions;
use GenericDatabase\Helpers\Parsers\XML as XmlParser;
...
class StructureHandler implements IStructure
{
    ...
    /**
     * Database directory path (folder containing XML files) or 'memory' for in-memory database
     * @var string $database
     */
    private static string $database = '';

    /**
     * Available tables (XML files in the database folder)
     * @var array $tables
     */
    ...
    /**
     * Get the available tables (XML files in the database directory).
     *
     * @return array
     */
    ...
    /**
     * Load data from a XML table file.
     *
     * @param string|null $table The table name (XML file without extension).
     * @return array
     * @throws Exceptions
     */
    public function load(?string $table = null): array
    {
        ...
        $data = XmlParser::parseTableXmlString($content);
    ...
    public function save(array $data, ?string $table = null): bool
    {
        ...
        $filePath = $this->getTablePath(self::$currentTable);
        $flags = XML::getDefaultEncodingFlags();
        $content = XmlParser::encodeTableToXml($data, $flags);
    ...
    /**
     * Scan the database directory for XML files (tables).
     *
     * @return array List of table names (without .xml extension).
     */
    private function scanTables(): array
    {
        ...
        if (!empty($this->get('database')) && is_dir($this->get('database'))) {
            $files = glob($this->get('database') . DIRECTORY_SEPARATOR . '*.xml');
