...
namespace GenericDatabase\Engine\XML\QueryBuilder;
...
/**
 * Builder class for XML QueryBuilder.
 * Converts query object to executable operations on XML data.
 *
 * @package GenericDatabase\Engine\XML\QueryBuilder
 */
class Builder implements IBuilder
{
    ...
    private function buildWhere(): array
    {
        ...
        foreach ($this->query->where as $data) {
            ...
            if ($aggregationType === Where::LIKE()) {
                $pattern = $default ?? '';
                $regex = XML::regex(Regex::likeToRegex((string) $pattern));
                $conditions[] = [
                    'column' => $column,
                    'alias' => $alias,
    ...
    private function buildOrderBy(): ?array
    {
        ...
        $direction = match ($order['sorting'] ?? null) {
            Sorting::DESCENDING() => XML::DESC,
            default => XML::ASC
        };
        if ($expr !== '' && preg_match('/^(.+?)\s+(ASC|DESC)\s*$/i', $expr, $m)) {
            $expr = trim($m[1]);
            $direction = strtoupper($m[2]) === 'DESC' ? XML::DESC : XML::ASC;
        }
     ...
     * Used by the XML engine so parseAndExecuteQuery receives unquoted identifiers.
     *
     * @return string
     * @throws Exceptions
     */
