...
/**
 * Builder class for YAML QueryBuilder.
 * Converts query object to executable operations on YAML data.
 *
...
class Builder implements IBuilder
{
    ...
    private function buildWhere(): array
    {
        ...
        foreach ($this->query->where as $data) {
            ...
            if ($aggregationType === Where::LIKE()) {
                $pattern = $default ?? '';
                $regex = YAML::regex(Regex::likeToRegex((string) $pattern));
                $conditions[] = [
                    'column' => $column,
                    'alias' => $alias,
    ...
    private function buildOrderBy(): ?array
    {
        ...
        $direction = match ($order['sorting'] ?? null) {
            Sorting::DESCENDING() => YAML::DESC,
            default => YAML::ASC
        };
        if ($expr !== '' && preg_match('/^(.+?)\s+(ASC|DESC)\s*$/i', $expr, $m)) {
            $expr = trim($m[1]);
            $direction = strtoupper($m[2]) === 'DESC' ? YAML::DESC : YAML::ASC;
        }
     ...
     * Used by the YAML engine so parseAndExecuteQuery receives unquoted identifiers.
     *
     * @return string
     * @throws Exceptions
     */
