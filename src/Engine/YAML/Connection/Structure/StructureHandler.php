...
namespace GenericDatabase\Engine\YAML\Connection\Structure;
...
class StructureHandler implements IStructure
{
    ...
    /**
     * Database directory path (folder containing YAML files) or 'memory' for in-memory database
     * @var string $database
     */
    private static string $database = '';

    /**
     * Available tables (YAML files in the database folder)
     * @var array $tables
     */
    ...
    /**
     * Current data from the YAML file
     * @var array $data
     */
    ...
    /**
     * Get the available tables (YAML files in the database directory).
     *
     * @return array
     */
    ...
    /**
     * Load data from a YAML table file.
     *
     * @param string|null $table The table name (YAML file without extension).
     * @return array
     * @throws Exceptions
     */
    public function load(?string $table = null): array
    {
        ...
        $data = YamlParser::parseTableYamlString($content);
    ...
    /**
     * Save data to a YAML table file.
     *
    ...
    public function save(array $data, ?string $table = null): bool
    {
        ...
        $filePath = $this->getTablePath(self::$currentTable);
        $flags = YAML::getDefaultEncodingFlags();
        $content = YamlParser::encodeTableToYaml($data, $flags);
    ...
    /**
     * Scan the database directory for YAML files (tables).
     *
     * @return array List of table names (without .yaml extension).
     */
    private function scanTables(): array
    {
        ...
        if (!empty($this->get('database')) && is_dir($this->get('database'))) {
            $files = glob($this->get('database') . DIRECTORY_SEPARATOR . '*.yaml');
