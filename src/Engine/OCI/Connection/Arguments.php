...
class Arguments
{
     ...
     */
    private static function callArgumentsByFormat(string $format, mixed $arguments): OCIConnection
    {
        $data = match ($format) {
            ...
            foreach ($data as $key => $value) {
                if (mb_strtolower($key) === 'options') {
                    call_user_func_array(
                        [OCIConnection::getInstance(), 'set' . ucfirst($key)],
     ...
     * @return OCIConnection
     */
    private static function callWithByStaticArray(array $arguments): OCIConnection
    {
        foreach ($arguments as $key => $value) {
            call_user_func_array([OCIConnection::getInstance(), 'set' . ucfirst($key)], [$value]);
        }
        return OCIConnection::getInstance();
    }
     ...
     * @noinspection PhpUnused
     */
    private static function callWithByStaticArgs(array $arguments): OCIConnection
    {
     ...
     * @throws ReflectionException
     */
    public static function call(string $name, array $arguments): OCIConnection
    {
        ...
        return match ($name) {
            ...
            default => call_user_func_array([OCIConnection::getInstance(), $name], $arguments)
        };
    }
}
