...
class Options
{
        ...
        foreach (Reflections::getClassConstants($class) as $key => $value) {
            ...
            if ($index !== false) {
                $keyName = $key !== 'ATTR_PERSISTENT' && $key !== 'ATTR_CONNECT_TIMEOUT' && $key !== 'ATTR_AUTOCOMMIT'
                    ...
                    : $key;
                OCIConnection::getInstance()->setAttribute("OCI::$key", $options[$value]);
                if ($key !== 'ATTR_PERSISTENT' && $key !== 'ATTR_CONNECT_TIMEOUT' && $key !== 'ATTR_AUTOCOMMIT') {
                    OCIConnection::getInstance()->setAttribute($keyName, $options[$value]);
                }
                if (str_contains($options[$value], 'FETCH')) {
                    self::$options[constant("$class::$key")] =
                        constant("$class::" . str_replace("OCI::", '', $options[$value]));
                } else {
     ...
     * more information's in https://www.php.net/manual/en/oci8.configuration.php
     *
     * @return void
     */
