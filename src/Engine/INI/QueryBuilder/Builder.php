...
/**
 * Builder class for INI QueryBuilder.
 * Converts query object to executable operations on INI data.
 *
...
class Builder implements IBuilder
{
    ...
    private function buildWhere(): array
    {
        ...
        foreach ($this->query->where as $data) {
            ...
            if ($aggregationType === Where::LIKE()) {
                $pattern = $default ?? '';
                $regex = INI::regex(Regex::likeToRegex((string) $pattern));
                $conditions[] = [
                    'column' => $column,
                    'alias' => $alias,
    ...
    private function buildOrderBy(): ?array
    {
        ...
        $direction = match ($order['sorting'] ?? null) {
            Sorting::DESCENDING() => INI::DESC,
            default => INI::ASC
        };
        if ($expr !== '' && preg_match('/^(.+?)\s+(ASC|DESC)\s*$/i', $expr, $m)) {
            $expr = trim($m[1]);
            $direction = strtoupper($m[2]) === 'DESC' ? INI::DESC : INI::ASC;
        }
     ...
     * Used by the INI engine so parseAndExecuteQuery receives unquoted identifiers.
     *
     * @return string
     * @throws Exceptions
     */
