...
namespace GenericDatabase\Engine\INI\Connection\Fetch;
...
/**
 * Handles fetch operations for INI connections.
 * Extends AbstractFlatFileFetch to leverage common flat-file fetch functionality.
 *
 * @package GenericDatabase\Engine\INI\Connection\Fetch
 */
class FetchHandler extends AbstractFlatFileFetch implements IFlatFileFetch
{
    ...
    {
        $fetch = $fetchStyle ?? INI::FETCH_ASSOC;

        return match ($fetch) {
            INI::FETCH_OBJ => $this->internalFetchAssoc() !== false ? (object) $this->getResultSet()[$this->cursor - 1] : false,
            INI::FETCH_INTO => $this->fetchIntoObject($fetchArgument),
            INI::FETCH_CLASS => $this->internalFetchClass($optArgs, $fetchArgument ?? '\stdClass'),
            INI::FETCH_COLUMN => $this->internalFetchColumn($fetchArgument ?? 0),
            INI::FETCH_ASSOC => $this->internalFetchAssoc(),
            INI::FETCH_NUM => $this->internalFetchNum(),
    ...
    public function fetchAll(?int $fetchStyle = null, mixed $fetchArgument = null, mixed $optArgs = null): array|bool
    {
        ...
        return match ($fetch) {
            INI::FETCH_OBJ => array_map(fn($row) => (object) $row, $this->internalFetchAllAssoc()),
            INI::FETCH_INTO => $this->fetchAllIntoObjects($fetchArgument),
            INI::FETCH_CLASS => $this->internalFetchAllClass($optArgs, $fetchArgument ?? '\stdClass'),
            INI::FETCH_COLUMN => $this->internalFetchAllColumn($fetchArgument ?? 0),
            INI::FETCH_ASSOC => $this->internalFetchAllAssoc(),
            INI::FETCH_NUM => $this->internalFetchAllNum(),
