...
namespace GenericDatabase\Engine\INI\Connection\Report;
...
class ReportHandler implements IReport
{
    ...
    public function handleError($message = null): void
    {
        ...
        if ($errorCode !== 0) {
            if ($this->getReportMode() & INI::REPORT_STRICT) {
                throw new Exception($errorMsg, $errorCode);
            } elseif ($this->getReportMode() & INI::REPORT_ERROR) {
                trigger_error($errorMsg, E_USER_WARNING);
            }
        } elseif ($message && $this->getReportMode() & INI::REPORT_STRICT) {
