...
namespace GenericDatabase\Engine\INI\Connection\DSN;
...
class DSNHandler implements IDSN
{
    ...
    private function handleINI(bool $isMemory = false): string
    {

        return vsprintf(
            ($isMemory) ? "ini::%s:" : "ini:%s",
            [
                $this->get('database')
            ]
        );
    ...
    public function parse(): string|Exceptions
    {
        $this->set(
            ...
            match ($this->get('database')) {
                'memory' => $this->handleINI(true),
                default => $this->handleINI(false),
            }
        );
