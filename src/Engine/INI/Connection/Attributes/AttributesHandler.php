...
namespace GenericDatabase\Engine\INI\Connection\Attributes;
...
class AttributesHandler extends AbstractAttributes implements IAttributes
{
    ...
    /**
     * Define all INI attribute of the connection a ready exist
     *
     * @return void
     * @throws Exceptions
     */
    public function define(): void
    {
        ...
        foreach ($keys as $key) {
            ...
            $result[$attribute] = match ($attribute) {
                'AUTOCOMMIT' => (bool)$this->getOptionsHandler()->getOptions(INI::ATTR_AUTOCOMMIT),
                'CASE' => 0,
                'ERRMODE' => 1,
                'CONNECTION_STATUS' => $this->connectionStatus(),
                'PERSISTENT' => (bool)$this->getOptionsHandler()->getOptions(INI::ATTR_PERSISTENT),
                'TIMEOUT' =>  (int) $this->getOptionsHandler()->getOptions(INI::ATTR_CONNECT_TIMEOUT) ?: 30,
                'EMULATE_PREPARES' => true,
                'DEFAULT_FETCH_MODE' => $this->getOptionsHandler()->getOptions(INI::ATTR_DEFAULT_FETCH_MODE) ?? INI::FETCH_BOTH,
