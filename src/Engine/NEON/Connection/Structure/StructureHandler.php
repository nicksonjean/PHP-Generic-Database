...
namespace GenericDatabase\Engine\NEON\Connection\Structure;
...
class StructureHandler implements IStructure
{
    ...
    /**
     * Database directory path (folder containing NEON files) or 'memory' for in-memory database
     * @var string $database
     */
    private static string $database = '';

    /**
     * Available tables (NEON files in the database folder)
     * @var array $tables
     */
    ...
    /**
     * Current data from the NEON file
     * @var array $data
     */
    ...
    /**
     * Get the available tables (NEON files in the database directory).
     *
     * @return array
     */
    ...
    /**
     * Load data from a NEON table file.
     *
     * @param string|null $table The table name (NEON file without extension).
     * @return array
     * @throws Exceptions
     */
    public function load(?string $table = null): array
    {
        ...
        $data = NeonParser::parseTableNeonString($content);
    ...
    /**
     * Save data to a NEON table file.
     *
    ...
    public function save(array $data, ?string $table = null): bool
    {
        ...
        $filePath = $this->getTablePath(self::$currentTable);
        $flags = NEON::getDefaultEncodingFlags();
        $content = NeonParser::encodeTableToNeon($data, $flags);
    ...
    /**
     * Scan the database directory for NEON files (tables).
     *
     * @return array List of table names (without .neon extension).
     */
    private function scanTables(): array
    {
        ...
        if (!empty($this->get('database')) && is_dir($this->get('database'))) {
            $files = glob($this->get('database') . DIRECTORY_SEPARATOR . '*.neon');
