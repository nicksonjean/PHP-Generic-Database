...
namespace GenericDatabase\Engine\NEON\Connection\DSN;
...
class DSNHandler implements IDSN
{
    ...
    private function handleNEON(bool $isMemory = false): string
    {

        return vsprintf(
            ($isMemory) ? "neon::%s:" : "neon:%s",
            [
                $this->get('database')
            ]
        );
    ...
    public function parse(): string|Exceptions
    {
        $this->set(
            ...
            match ($this->get('database')) {
                'memory' => $this->handleNEON(true),
                default => $this->handleNEON(false),
            }
        );
