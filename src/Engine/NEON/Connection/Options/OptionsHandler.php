...
namespace GenericDatabase\Engine\NEON\Connection\Options;
...
class OptionsHandler extends AbstractOptions implements IOptions
{
        ...
        foreach (Reflections::getClassConstants($class) as $key => $value) {
            ...
            if ($index !== false) {
                $keyName = $key !== 'ATTR_PERSISTENT' && $key !== 'ATTR_CONNECT_TIMEOUT' && $key !== 'ATTR_AUTOCOMMIT'
                    ? str_replace("ATTR", "NEON", $key)
                    : $key;
                $this->getInstance()->setAttribute("NEON::$key", $options[$value]);
                ...
                if (str_contains($options[$value], 'FETCH')) {
                    self::$options[constant("$class::$key")] = constant("$class::" . str_replace("NEON::", '', $options[$value]));
                } else {
     ...
     * Note: NEON engine has no native PHP extension; persistence is file-based.
     *
     * @return void
     */
    public function define(): void
    {
        // No native neon extension options to set for file-based NEON engine
    }
}
