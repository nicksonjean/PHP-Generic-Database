...
namespace GenericDatabase\Engine\NEON\Connection\Fetch;
...
/**
 * Handles fetch operations for NEON connections.
 * Extends AbstractFlatFileFetch to leverage common flat-file fetch functionality.
 *
 * @package GenericDatabase\Engine\NEON\Connection\Fetch
 */
class FetchHandler extends AbstractFlatFileFetch implements IFlatFileFetch
{
    /**
     * Cache of loaded table data by table name (per executeStoredQuery) to avoid re-loading/re-parsing
    ...
    {
        $fetch = $fetchStyle ?? NEON::FETCH_ASSOC;

        return match ($fetch) {
            NEON::FETCH_OBJ => $this->internalFetchAssoc() !== false ? (object) $this->getResultSet()[$this->cursor - 1] : false,
            NEON::FETCH_INTO => $this->fetchIntoObject($fetchArgument),
            NEON::FETCH_CLASS => $this->internalFetchClass($optArgs, $fetchArgument ?? '\stdClass'),
            NEON::FETCH_COLUMN => $this->internalFetchColumn($fetchArgument ?? 0),
            NEON::FETCH_ASSOC => $this->internalFetchAssoc(),
            NEON::FETCH_NUM => $this->internalFetchNum(),
    ...
    public function fetchAll(?int $fetchStyle = null, mixed $fetchArgument = null, mixed $optArgs = null): array|bool
    {
        ...
        return match ($fetch) {
            NEON::FETCH_OBJ => array_map(fn($row) => (object) $row, $this->internalFetchAllAssoc()),
            NEON::FETCH_INTO => $this->fetchAllIntoObjects($fetchArgument),
            NEON::FETCH_CLASS => $this->internalFetchAllClass($optArgs, $fetchArgument ?? '\stdClass'),
            NEON::FETCH_COLUMN => $this->internalFetchAllColumn($fetchArgument ?? 0),
            NEON::FETCH_ASSOC => $this->internalFetchAllAssoc(),
            NEON::FETCH_NUM => $this->internalFetchAllNum(),
