 ...
 */
class StatementOperationHandler extends AbstractStatements implements IStatementOperations
{
    ...
    private function prepareStatement(mixed ...$params): PDOStatement|false
    {
        ...
        if (!empty($params)) {
            ...
            $statement = PDOConnection::getInstance()->getConnection()->prepare($this->parse(...$params), $cursor);
            if ($statement) {
     ...
     */
    public function query(mixed ...$params): ?PDOConnection
    {
        if (!empty($params) && ($statement = $this->prepareStatement(...$params)) && $this->exec($statement)) {
            ...
            if ($colCount > 0) {
                ...
                $this->setQueryRows(in_array($driver, ['oci', 'firebird', 'sqlite'])
     ...
     * @return PDOConnection|null
     */
    public function prepare(mixed ...$params): ?PDOConnection
    {
