...
use GenericDatabase\Engine\YAML\QueryBuilder\Builder;
use GenericDatabase\Engine\YAML\QueryBuilder\Clause;
...
class YAMLQueryBuilder implements IQueryBuilder
{
    ...
    private function runOnce(): void
    {
        ...
        if (self::$lastQuery !== $currentQueryForDisplay || self::$cursorExhausted) {
            ...
            $rows = $conn->fetchAll(YAML::FETCH_ASSOC);
    ...
    private function formatRow(mixed $row, int $fetchStyle, mixed $fetchArgument = null, mixed $optArgs = null): mixed
    {
        ...
        return match ($fetchStyle) {
            YAML::FETCH_NUM => array_values($row),
            YAML::FETCH_BOTH => $this->formatBothMode($row),
            YAML::FETCH_OBJ => (object) $row,
            YAML::FETCH_COLUMN => $fetchArgument !== null
                ? ($row[$fetchArgument] ?? array_values($row)[0] ?? null)
                : (array_values($row)[0] ?? null),
            YAML::FETCH_CLASS => $this->fetchClass($row, $fetchArgument, $optArgs),
            YAML::FETCH_INTO => $this->fetchInto($row, $fetchArgument),
            default => $row,
        };
    }
