...
class Builder implements IBuilder
{
    ...
    /**
     * Build only SELECT and FROM (no WHERE). Used for EXISTS subquery SQL so we never emit ? for correlated refs.
     *
     * @return string
     */
    ...
    /**
     * Resolve outer reference (e.g. "users.name") from context row for correlated subqueries.
     *
     * @param mixed $value Value possibly referencing outer column.
     * @param array $contextRow Current outer row (e.g. ['name' => 'Alice']).
    ...
    /**
     * Execute query on data and return results.
     *
    ...
    {
        // First, execute the main query
        $mainResults = $this->executeSingle($data, $contextRow);

        // If there are UNION clauses, execute them and merge results
        if (!empty($this->query->union) || !empty($this->query->unionAll)) {
            ...
            if (!empty($this->query->union)) {
                foreach ($this->query->union as $union) {
                    $unionResults = $this->executeUnionQuery($union, $data);
                    $allResults[] = $unionResults;
                }
            }
            
            // Execute UNION ALL queries
            if (!empty($this->query->unionAll)) {
                foreach ($this->query->unionAll as $unionAll) {
                    $unionAllResults = $this->executeUnionQuery($unionAll, $data);
                    $allResults[] = $unionAllResults;
                }
            }
    ...
    /**
     * Execute a single query without UNION processing.
     *
     * @param array $data The data to query.
     * @return array The query results.
     * @throws Exceptions
    ...
    private function executeSingle(array $data, array $contextRow = []): array
    {
        ...
        // Apply WHERE (non-EXISTS conditions; resolve outer refs from contextRow for subqueries)
        $where = $this->buildWhere($contextRow);
        if (!empty($where)) {
            $processor->where($where, $this->getWhereLogic());
        }

        // Apply EXISTS / NOT EXISTS filter (Fase 1: execution of subqueries)
    ...
    /**
     * Apply WHERE EXISTS / NOT EXISTS filter by running each subquery (per-row for correlated).
     *
     * @param array $data Candidate rows after non-EXISTS WHERE.
     ...
     *
     * @param array $union The union query data.
     * @param array $data The original data.
     * @return array The union query results.
     * @throws Exceptions
     */
    private function executeUnionQuery(array $union, array $data): array
    {
        if ($union['type'] === 'subquery') {
            // Execute subquery
            return $union['query']->execute();
        } else {
            // For raw SQL queries in flat files, we'll need to parse and execute
            // This is complex and may require a full SQL parser
     ...
     *
     * @param array $resultSets Array of result sets to merge.
     * @param bool $removeDuplicates Whether to remove duplicates (UNION) or not (UNION ALL).
    ...
    private function mergeResults(array $resultSets, bool $removeDuplicates): array
    {
        ...
        foreach ($resultSets as $results) {
            foreach ($results as $row) {
                // Convert row to serialized string for comparison
                $key = serialize($row);
                ...
                if ($removeDuplicates) {
                    ...
                    // For UNION ALL, include all rows
                    $merged[] = $row;
                }
            }
        }
    ...
    public function build(): string
    {
        ...
        if (!$hasUnion) {
            // ORDER BY and LIMIT only on main SELECT when no UNION
        ...
        if ($hasUnion) {
            // ORDER BY and LIMIT apply to whole UNION result
            $orderByStr = $this->buildOrderByString();
            if ($orderByStr !== '') {
                $parts[] = $orderByStr;
            }
    ...
    /**
     * Build EXISTS/NOT EXISTS fragment for WHERE clause.
     *
     * @return string
     */
    /**
     * Build WHERE fragment from a query's raw where array (for EXISTS subquery).
     * Used when subquery->build() omits WHERE so we append it reliably.
     *
    ...
    /**
     * Append UNION / UNION ALL to the parts array.
     *
     * @param array $parts The parts array (passed by reference).
     */
     ...
     */
    /**
     * Collect literal values from a query's select columns (for getValues order).
     *
     * @param object $query Query with select['columns'].
     * @return array
     */
    ...
    /**
     * Cast numeric strings to int or float in the arguments array.
     *
     * @param array $values
     * @return array
     */
