...
namespace GenericDatabase\Engine\CSV\Connection\DSN;
...
class DSNHandler implements IDSN
{
    ...
    private function handleCSV(bool $isMemory = false): string
    {

        return vsprintf(
            ($isMemory) ? "csv::%s:" : "csv:%s",
            [
                $this->get('database')
            ]
        );
    ...
    public function parse(): string|Exceptions
    {
        $this->set(
            ...
            match ($this->get('database')) {
                'memory' => $this->handleCSV(true),
                default => $this->handleCSV(false),
            }
        );
