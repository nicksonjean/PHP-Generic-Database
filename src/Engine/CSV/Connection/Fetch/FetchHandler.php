...
class FetchHandler extends AbstractFlatFileFetch implements IFlatFileFetch
{
    ...
    /**
     * Fallback split when SelectParser returns one segment but query contains UNION.
     *
    ...
    private function extractWhereGroupHavingOrderLimit(string $s, array &$out): void
    {
        ...
        if (preg_match('/\bHAVING\s+(.+?)(?=\s+ORDER\s+BY|\s+LIMIT|\s+ROWS|$)/is', $s, $m)) {
            ...
            } elseif (preg_match('/^(\w+)\s*(>=|<=|!=|<>|>|<|=)\s*(\d+\.?\d*)\s*$/i', $hav, $hm)) {
                $having = ['alias' => trim($hm[1]), 'op' => $hm[2], 'val' => strpos((string) $hm[3], '.') !== false ? (float) $hm[3] : (int) $hm[3]];
            }
        }
    ...
    private function executeGroupByAggregate(
        ...
        foreach ($groups as $g) {
            ...
            if ($having !== null) {
                ...
                $havVal = $having['val'];
                $candidate = null;
                if (isset($having['alias'])) {
                    $candidate = $outRow[$having['alias']] ?? null;
                } else {
                    $aggCol = $having['agg'];
    ...
    public function fetch(?int $fetchStyle = null, mixed $fetchArgument = null, mixed $optArgs = null): mixed
    {
        ...
        return match ($fetch) {
            CSV::FETCH_OBJ => $this->internalFetchAssoc() !== false ? (object) $this->getResultSet()[$this->cursor - 1] : false,
