...
use GenericDatabase\Engine\CSV\QueryBuilder\Regex;

/**
 * Builder class for CSV QueryBuilder.
 * Converts query object to executable operations on CSV data.
 *
...
class Builder implements IBuilder
{
    ...
    private function buildWhere(array $contextRow = []): array
    {
        ...
        foreach ($this->query->where as $data) {
            ...
            if ($aggregationType === Where::LIKE()) {
                $pattern = $default ?? '';
                $regex = CSV::regex(Regex::likeToRegex((string) $pattern));
                $conditions[] = [
                    'column' => $column,
                    'alias' => $alias,
    ...
    private function buildOrderBy(): ?array
    {
        ...
        $direction = match ($order['sorting'] ?? null) {
            Sorting::DESCENDING() => CSV::DESC,
            default => CSV::ASC
        };
        if ($expr !== '' && preg_match('/^(.+?)\s+(ASC|DESC)\s*$/i', $expr, $m)) {
            $expr = trim($m[1]);
            $direction = strtoupper($m[2]) === 'DESC' ? CSV::DESC : CSV::ASC;
        }
     ...
     * Used by the CSV engine so parseAndExecuteQuery receives unquoted identifiers.
     *
     * @return string
     * @throws Exceptions
     */
