...
class Arguments
{
     ...
     */
    private static function callArgumentsByFormat(string $format, mixed $arguments): SQLSrvConnection
    {
        $data = match ($format) {
            ...
            foreach ($data as $key => $value) {
                if (mb_strtolower($key) === 'options') {
                    call_user_func_array(
                        [SQLSrvConnection::getInstance(), 'set' . ucfirst($key)],
     ...
     * @return SQLSrvConnection
     */
    private static function callWithByStaticArray(array $arguments): SQLSrvConnection
    {
        foreach ($arguments as $key => $value) {
            call_user_func_array([SQLSrvConnection::getInstance(), 'set' . ucfirst($key)], [$value]);
        }
        return SQLSrvConnection::getInstance();
    }
     ...
     * @noinspection PhpUnused
     */
    private static function callWithByStaticArgs(array $arguments): SQLSrvConnection
    {
     ...
     * @throws ReflectionException
     */
    public static function call(string $name, array $arguments): SQLSrvConnection
    {
        ...
        return match ($name) {
            ...
            default => call_user_func_array([SQLSrvConnection::getInstance(), $name], $arguments)
        };
    }
}
