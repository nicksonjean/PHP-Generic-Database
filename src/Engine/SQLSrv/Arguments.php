...
class Arguments
{
  ...
  private static function callWithFullArguments($arguments): void
  {
    foreach ($arguments as $key => $value) {
      call_user_func_array([SQLSrvEngine::getInstance(), 'set' . self::$argumentList[$key]], [$value]);
    }
  }
   ...
   * @return array 
   */
  private static function setConstant($value, $attributes = ['ATTR_PERSISTENT', 'ATTR_CONNECT_TIMEOUT']): array
  {
    $options = Types::setConstant($value, SQLSrvEngine::getInstance(), 'SQLSrv', 'SQLSrv', ['ATTR_PERSISTENT', 'ATTR_CONNECT_TIMEOUT']);
  ...
  {
    call_user_func_array([SQLSrvEngine::getInstance(), $method], $arguments);
  }
