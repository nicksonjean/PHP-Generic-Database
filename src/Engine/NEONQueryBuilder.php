...
use GenericDatabase\Engine\NEON\QueryBuilder\Builder;
use GenericDatabase\Engine\NEON\QueryBuilder\Clause;

/**
 * The `NEONQueryBuilder` class implements the `IQueryBuilder` interface to provide a flexible
 * query building mechanism for NEON flat file databases.
 *
 * @package GenericDatabase\Engine
 */
class NEONQueryBuilder implements IQueryBuilder
{
    use Query;
    use Context;
    use Singleton;

    private static NEONQueryBuilder $self;
    ...
    private function runOnce(): void
    {
        ...
        if (self::$lastQuery !== $currentQueryForDisplay || self::$cursorExhausted) {
            ...
            $rows = $conn->fetchAll(NEON::FETCH_ASSOC);
    ...
    private function formatRow(mixed $row, int $fetchStyle, mixed $fetchArgument = null, mixed $optArgs = null): mixed
    {
        ...
        return match ($fetchStyle) {
            NEON::FETCH_NUM => array_values($row),
            NEON::FETCH_BOTH => $this->formatBothMode($row),
            NEON::FETCH_OBJ => (object) $row,
            NEON::FETCH_COLUMN => $fetchArgument !== null
                ? ($row[$fetchArgument] ?? array_values($row)[0] ?? null)
                : (array_values($row)[0] ?? null),
            NEON::FETCH_CLASS => $this->fetchClass($row, $fetchArgument, $optArgs),
            NEON::FETCH_INTO => $this->fetchInto($row, $fetchArgument),
            default => $row,
        };
    }
